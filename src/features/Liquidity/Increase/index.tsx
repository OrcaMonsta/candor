      ...
      <Grid templateColumns={['unset', '.5fr .8fr .6fr']} gap={'clamp(16px, 1.5vw, 64px)'} mt={8}>
        ...
        <GridItem>
          <VStack spacing={4}>
            ...
            <Box {...panelCard} bg={colors.backgroundSwapCardsTransparent13} border={colors.cardBorder02} backdropFilter={"blur(5px)"} borderRadius="20px" overflow="hidden" w="full">
              <Tabs isFitted items={tabOptions} size="md" variant="folder" value={tabValue} onChange={handleTabChange} borderBottom={colors.cardBorder02} />
              {mode === 'add' ? (
                <AddLiquidity
                  pool={pool}
                  isLoading={isLoading}
