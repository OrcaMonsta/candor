  ...
  return (
    <Box position={'relative'} rounded={12} sx={ctrSx}>
      ...
      <HStack
        ...
        px={colorMode == "dark" ? sizes.downerUpperGridPx : 3}
        py={sizes.upperGridPy}
        {...(topBlockSx || {})}
      >
        {/* top left label */}

        {colorMode !== "dark" && topLeftLabel
          ?
          <Button
            size="xs"
            height="fit-content"
      ...
      {colorMode !== "dark"
        ...
        <Box>
          <HStack
            bg={disableSelectToken ? undefined : "transparent"}
            rounded={disableSelectToken ? undefined : 12}
            px={disableSelectToken ? undefined : 3}
            py={disableSelectToken ? undefined : 2.5}
            cursor={disableSelectToken ? undefined : 'pointer'}
          ...
          >
            {hideTokenIcon ? null : (
              <TokenAvatar ml={tokenIconMarginL ? tokenIconMarginL : "unset"} token={token} size={disableSelectToken ? sizes.disableSelectTokenIconSize : sizes.tokenIcon} />
            )}
            <Text color={isLight ? colors.secondary : colors.textPrimary}>{token?.symbol || ' '}</Text>
            {disableSelectToken ? undefined : <ChevronDownIcon width={20} height={20} />}
          </HStack>
